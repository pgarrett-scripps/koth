...
use crate::scoring::model::PEPTIDE;

#[test]
fn peptide_counts_scale_linearly() {
    // 1500 Da peptide → averagine integer composition
    let (c, h, n, o, s) = PEPTIDE.counts(1500.0);
    let scale = 1500.0 / PEPTIDE.residue_mass;
...
#[test]
fn peptide_distribution_sums_to_one() {
    for mass in [500.0, 1500.0, 3000.0, 5000.0] {
        let d = PEPTIDE.distribution(mass);
...
fn offsets_resolve_against_ceil_bands() {
    ...
    for mass in [500.0, 1500.0, 2600.0] {
        assert_eq!(PEPTIDE.sulfur_ceil(mass), 1, "mass {mass}");
        assert_eq!(resolve_sulfur_counts(&PEPTIDE, mass, &d), vec![0, 1, 2]);
    }
    for mass in [2700.0, 4000.0, 5300.0] {
        assert_eq!(PEPTIDE.sulfur_ceil(mass), 2, "mass {mass}");
        assert_eq!(resolve_sulfur_counts(&PEPTIDE, mass, &d), vec![1, 2, 3]);
    }
    for mass in [5400.0, 7000.0, 7900.0] {
        assert_eq!(PEPTIDE.sulfur_ceil(mass), 3, "mass {mass}");
        assert_eq!(resolve_sulfur_counts(&PEPTIDE, mass, &d), vec![2, 3, 4]);
    }
}
...
fn offsets_saturate_and_dedup() {
    assert_eq!(
        resolve_sulfur_counts(&PEPTIDE, 1500.0, &[-3, -2, -1, 0, 1]),
        vec![0, 1, 2]
    );
    assert_eq!(resolve_sulfur_counts(&PEPTIDE, 1500.0, &[0, 0, 0]), vec![1]);
    assert_eq!(
        resolve_sulfur_counts(&PEPTIDE, 1500.0, &[]),
        Vec::<u32>::new()
    );
    // Order is preserved as declared, not sorted.
    assert_eq!(
        resolve_sulfur_counts(&PEPTIDE, 4000.0, &[1, -1, 0]),
        vec![3, 1, 2]
    );
}
...
fn default_offsets_reach_two_over_tryptic_band() {
    ...
    for mass in [800.0, 1332.0, 2000.0, 3000.0, 3997.0] {
        let v = resolve_sulfur_counts(&PEPTIDE, mass, &d);
        assert!(v.contains(&2), "mass {mass} set {v:?} skips n_S = 2");
    }
    // Widening by one recovers the no-sulfur template above 2665 Da.
    assert!(!resolve_sulfur_counts(&PEPTIDE, 3000.0, &d).contains(&0));
    assert!(resolve_sulfur_counts(&PEPTIDE, 3000.0, &[-2, -1, 0, 1]).contains(&0));
}
...
fn scoring_is_max_over_resolved_set() {
    ...
    for mass in [900.0, 2000.0, 3000.0, 6000.0] {
        let (c, h, n, o, _) = PEPTIDE.counts(mass);
        let manual = resolve_sulfur_counts(&PEPTIDE, mass, &d)
            .iter()
            .map(|&s| bhattacharyya_score(&obs, &cache().distribution(c, h, n, o, s)))
            .fold(0.0f64, f64::max);
        let got = bhattacharyya_score_best_sulfur(&obs, mass, &d, &PEPTIDE).0;
...
fn empty_offsets_match_plain_template() {
    ...
    for mass in [900.0, 1500.0, 3000.0, 6000.0] {
        let plain = bhattacharyya_score(&obs, &PEPTIDE.distribution(mass));
        let got = bhattacharyya_score_best_sulfur(&obs, mass, &[], &PEPTIDE).0;
...
fn sulfur_override_changes_pattern() {
    let m = 1500.0;
    let avg = PEPTIDE.distribution(m);
    let high_s = PEPTIDE.distribution_with_sulfur(m, 3);
    assert!(
