...
pub fn detect_hills_from_iter<I>(spectra: I, config: &HillsConfig) -> Vec<Hill>
where
    I: Iterator<Item = Spectrum>,
{
    let mut det = HillDetector::new(config);
...
pub fn detect_hills(spectra: &[Spectrum], config: &HillsConfig) -> Vec<Hill> {
    log::info!("Starting hill detection on {} spectra", spectra.len());
    detect_hills_from_iter(spectra.iter().cloned(), config)
}
